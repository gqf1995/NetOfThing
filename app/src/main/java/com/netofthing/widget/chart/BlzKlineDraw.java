...
public class BlzKlineDraw {
    ...
    public void isFullScreen(boolean isFull) {
        ...
        mChartKline.setMaxVisibleValueCount(60);
        mChartVolume.setMaxVisibleValueCount(60);
    }
    ...
    public void setKlineShowType(int selectType) {
        ...
        setOffset();
        mChartKline.postInvalidateDelayed(200);
        mChartVolume.postInvalidateDelayed(200);
    }
...
//    }
    ...
    public void updata(List<KLineBean> lineBeans) {
        if (lineBeans.size() > 0) {
            ...
            lineBeans = mData.onlyParseKLine(lineBeans);
            ...
            updatLastKline(kLineBean, true);
            updatLastVolume(kLineBean, true);


            if (lineBeans.size() > 1) {
                for (int i = 1; i < lineBeans.size(); i++) {
                    if (TextUtils.isEmpty(lineBeans.get(i).date)) {
                        lineBeans.get(i).date = TimeUtils.millis2String(lineBeans.get(i).timestamp * 1000, mData.simpleDateFormat);
                    }
                    kLineDatas.add(lineBeans.get(i));
                    mData.getXVals().add(lineBeans.get(i).date);
                    mChartKline.getXAxis().getValues().add(lineBeans.get(i).date);
                ...
                for (int i = 1; i < lineBeans.size(); i++) {
                    ...
                    updatLastKline(lineBeans.get(i), false);
                    updatLastVolume(lineBeans.get(i), false);
                }
            }
    ...
    private void setVolumeByChart(CombinedChart combinedChart) {
        ...
        for (int i = 1; i < mData.getKLineDatas().size(); i++) {
            if (mData.getKLineDatas().get(i).getClose().doubleValue()
                    > mData.getKLineDatas().get(i - 1).getOpen().doubleValue()) {
...
//        //combinedChart.setVisibleXRange(minXRange, maxXRange);
    ...
    private void setChartListener() {
        ...
        //        mChartKline.setOnChartGestureListener(new OnChartGestureListener() { // 手势监听器
        //            @Override
        ...
        //                // 长按
        //                mChartKline.setDragEnabled(false);
