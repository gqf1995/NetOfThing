...
public class KCombinedChart extends CombinedChart {
    ...
    public void drawMarkerView() {
        ...
        if (dashMarkerView != null && myNowPriceMarkerView != null) {
        ...
        for (int i = 0; i < mIndicesToHighlight.length; i++) {
            ...
            if (xIndex <= deltaX && xIndex <= deltaX * mAnimator.getPhaseX()) {
                ...
                if (null != myBottomMarkerView) {
                    ...
                    time = time + "  " + com.blankj.utilcode.util.TimeUtils.getChineseWeek(kLineBean.getTimestamp() * 1000);
                                ...
                                if (myInfoMarkerView != null) {
                                    ...
                                    if (lineData != null) {
                                        ...
                                        if (dataSetByIndex != null) {
                                            myInfoMarkerView.setLine(dataSetByIndex.isVisible());
                                        }
                                    }
                                    myInfoMarkerView.refreshContent(e, mIndicesToHighlight[i]);
                                    myInfoMarkerView.measure(MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED),
                                            MeasureSpec.makeMeasureSpec(0, MeasureSpec.UNSPECIFIED));
                                    myInfoMarkerView.layout(0, 0, myInfoMarkerView.getMeasuredWidth(),
                                            myInfoMarkerView.getMeasuredHeight());

                                    mlWidth = mViewPortHandler.contentLeft() + myMarkerViewLeft.getWidth();

                                    float chartWidth = mViewPortHandler.getChartWidth();

                                    float v = chartWidth / (this.getHighestVisibleXIndex() - this.getLowestVisibleXIndex()) * (this.getHighestVisibleXIndex() - e.getXIndex());

                                    if (myInfoMarkerView.getMeasuredWidth() < v) {
                                        mlWidth = chartWidth - myInfoMarkerView.getMeasuredWidth() - CommonUtils.getDimensionPixelSize(R.dimen.trans_20px);
                                    }
                                    myInfoMarkerView.draw(canvas, mlWidth, CommonUtils.getDimensionPixelSize(R.dimen.trans_35px));
                                }
            }
        }
    }
